...
namespace FoodOrdersApi.Controllers
{
    ...
    public class MealController : ControllerBase
    {
        ...
        // GET api/meal/order/5
        [HttpGet("order/{id}")]
        public ActionResult GetFromOrder(int id)
        {
            var mealDtos = _mealService.GetFromOrder(id);

            return Ok(mealDtos);
        }

        // GET api/meal/restaurant/5
        [HttpGet("restaurant/{id}")]
        public ActionResult GetFromRestaurant(int id)
        {
            var mealDtos = _mealService.GetFromRestaurant(id);
